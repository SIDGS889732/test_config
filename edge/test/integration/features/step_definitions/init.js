...
var environment = config['${projectName}-${teamName}-test_config'].env;
...
console.log('&$${projectName}-${teamName}-test_config api: [' +  domains[environment] + ', ' + config['${projectName}-${teamName}-test_config'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['${projectName}-${teamName}-test_config'].app, config['${projectName}-${teamName}-test_config'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['${projectName}-${teamName}-test_config'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
